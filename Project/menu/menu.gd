...
@export var button_press_audio_player: AudioStreamPlayer
@export var button_hover_audio_player: AudioStreamPlayer
...
func _ready() -> void:
	...
	volume_slider.set_value_no_signal(Settings.config_file.get_value("audio", "main_volume", 0.5))
	music_slider.set_value_no_signal(Settings.config_file.get_value("audio", "music_volume", 1.0))
	effect_volume_slider.set_value_no_signal(Settings.config_file.get_value("audio", "effect_volume", 1.0))
	ui_volume_slider.set_value_no_signal(Settings.config_file.get_value("audio", "ui_volume", 1.0))
...
func _connect_ui_sounds(node: Node) -> void:
	if node is Button:
		node.pressed.connect(_play_button_press_sound)
		node.mouse_entered.connect(_play_button_hover_sound)
	for child_node: Node in node.get_children():
		_connect_ui_sounds(child_node)
...
func _on_volume_slider_value_changed(value: float) -> void:
	...
	Settings.config_file.set_value("audio", "main_volume", value)
	Settings.save_settings()


func _on_music_volume_slider_value_changed(value: float) -> void:
	AudioServer.set_bus_volume_linear(1, value)
	Settings.config_file.set_value("audio", "music_volume", value)
	Settings.save_settings()


func _on_effect_volume_slider_value_changed(value: float) -> void:
	AudioServer.set_bus_volume_linear(2, value) # SFX
	Settings.config_file.set_value("audio", "effect_volume", value)
	Settings.save_settings()


func _on_ui_volume_slider_value_changed(value: float) -> void:
	AudioServer.set_bus_volume_linear(3, value) # ui
	Settings.config_file.set_value("audio", "ui_volume", value)
	Settings.save_settings()
