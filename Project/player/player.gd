...
@export var step_timer_move: Timer
@export var step_timer_strafe: Timer
...
func apply_input(delta: float) -> void:
	...
	var is_moving: bool = motion.length() > 0.1
	var is_strafing: bool = player_input.aiming and is_moving
	...
	if is_on_floor():
		if airborne_time > 0.5:
			land.rpc()
			if is_strafing:
				step_timer_strafe.start()
			elif is_moving:
				step_timer_move.start()
		airborne_time = 0

	var on_air: bool = airborne_time > MIN_AIRBORNE_TIME

	if on_air:
		step_timer_move.stop()
		step_timer_strafe.stop()
	...
	if is_strafing != was_strafing and not on_air:
		if is_strafing:
			step_timer_strafe.start()
			step_timer_move.stop()
		else:
			...
			if is_moving:
				step_timer_move.start()
		was_strafing = is_strafing
	
	if is_moving != was_moving and not on_air:
		if is_moving and !is_strafing:
			step_timer_move.start()
		else:
			step_timer_move.stop()
